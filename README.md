...
#### Разрешения в «Конфиденциальность и безопасность»

Если вход в Google не завершается или браузер не открывается, откройте **Системные настройки → Конфиденциальность и безопасность** и выдайте приложению **Google Search Console Updater** дополнительные разрешения:

| Раздел | Зачем нужно |
|--------|-------------|
| **Локальная сеть** | OAuth-авторизация: приложение принимает ответ Google на `127.0.0.1` |
| **Автоматизация** | Открытие браузера для входа в Google (если macOS запросит доступ) |

После выдачи разрешений перезапустите приложение и повторите **Войти в Google**.
